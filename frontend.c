...
int init_frontend(short port_fe, struct sockaddr_in* self_addr){
  ...
  /* socket: create front end sockets */
  ...
  if (listen(sockfd_fe, FE_MAX_CON_RQSTS) < 0) /* allow 10 requests to queue up */
...
void frontend_response(int connfd, char* BUF, struct thread_data *ct) {
  char content_filepath[MAX_FILEPATH_LEN];
  ...
  bzero(content_filepath, MAX_FILEPATH_LEN);
  bzero(buf, BUFSIZE);
  bzero(path, MAXLINE);

  strcpy(content_filepath, ct->content_fp);
  strcpy(buf, BUF);
...
void fe_range_response(int connfd, char* BUF, struct thread_data *ct) {
   ...
   */
  int range_flag = parse_range_request(buf, sbyte, ebyte);

  /* In case this is actually normal GET rqt */
  if(range_flag == 0) {
    printf("parsed as get.\n");
  ...
  printf("Sending partial content to {Client %d}...", tid);
  ...
  printf("Sent %d (%d-%d) bytes to {Client %d}!", sent, sb, eb, tid);
