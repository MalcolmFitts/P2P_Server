...
#define DEF_CF_FOLDER "config/"
#define IP_STR_MAX_LEN 256

pthread_mutex_t mutex;

int log_lvl;                  /* Var defining how verbose the terminal log is */
...
int main(int argc, char **argv) {
  ...
  char* content_fp = NULL;

  /* check command line args */
  if (argc % 2 != 1) {
    fprintf(stderr, "usage: %s [-c] <config file name> [-v] <verbosity level>\n", argv[0]);
  ...
  if(argc == 3) {
    ...
    strcpy(config_filename, DEF_CF_FOLDER);
    strcat(config_filename, argv[2]);
    ...
    if(config_init_flag != 1) {
      ...
      config_filename = malloc(MAXLINE);
      strcpy(config_filename, DEF_CF_FOLDER);
      strcat(config_filename, CF_DEFAULT_FILENAME);
    }
  }

  /* Config file AND terminal log verbosity input */
  else if (argc == 5) {
    if((strcmp(argv[1], "-c") != 0) || (strcmp(argv[3], "-v") != 0)) {
      fprintf(stderr, "usage: %s [-c] <config file name> [-v] <verbosity level>\n", 
        argv[0]);
      exit(0);
    }
  ...
  /* Initializing mutex used for front-end/back-end comms */
  pthread_mutex_init(&mutex, NULL);

  /* Resolving front / backend ports */ 
  cf_port_fe = get_config_field(config_filename, CF_TAG_FE_PORT, 0);
  cf_port_be = get_config_field(config_filename, CF_TAG_BE_PORT, 0);

  /* Resolving content filepath from config file */
  content_fp = get_config_field(config_filename, CF_TAG_CONTENT_DIR, 0);
  ...
  if(!content_fp) {
    content_fp = malloc(MAXLINE);
    strcpy(content_fp, CF_DEFAULT_CONTENT_DIR);
  }
  
  /* initialize front-end and back-end sockets */
  sockfd_fe = init_frontend(port_fe, &self_addr_fe);
  sockfd_be = init_backend(port_be, &self_addr_be);
  
  log_startup(SERVER_NAME, config_filename, log_lvl);
...
void *serve_client_thread(void *ptr) {
  ...
  switch(rqt){
    ...
    case RQT_C_RNG:
      printf("Server recognized request type: HTTP Range Request\n");
      fe_range_response(connfd, buf, ct);
      break;

    default:
      /* Standard FE response */

      /* TO DO - split this into range and regular front end request. */
      printf("Server recognized request type: HTTP Request\n");
  ...
  // while((refresh_ctr < 100) 
  //         && ((n = read(connfd, buf, BUFSIZE)) <= 0)) {

  //   printf("DB----\t read from buf after dealing with FE request:\n");
